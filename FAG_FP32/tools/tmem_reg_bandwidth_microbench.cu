...
// used by the FMHA S -> register path.  Four atom loads/warp reproduce one
// 128x128 FP32 S tile across eight compute warps (64 KiB total).
template <int Warps, int Iterations>
__global__ void tmem_reg_bandwidth(Result* out) {
  ...
  const uint32_t taddr = tmem_base;
  uint32_t sink = 0;
  uint32_t warm[16];
  ...
  for (int iter = 0; iter < Iterations; ++iter) {
    ...
    load_32x32b_x16(taddr, a0);
    load_32x32b_x16(taddr, a1);
    load_32x32b_x16(taddr, a2);
    load_32x32b_x16(taddr, a3);
...
double run_bandwidth(Result* d_result, Result& h_result) {
  ...
  for (int trial = 0; trial < kTrials; ++trial) {
    ...
    tmem_reg_bandwidth<Warps, kIterations><<<1, Warps * 32>>>(d_result);
...
int main() {
  ...
  tmem_reg_bandwidth<1, 32><<<1, 32>>>(d_result);
  ...
  std::printf("warps  median effective B/cycle  cycles per 64KiB-equivalent tile\n");
  for (int warps : {1, 2, 4, 8, 16, 32}) {
    double bw = 0.0;
    if (warps == 1) bw = run_bandwidth<1>(d_result, h_result);
    if (warps == 2) bw = run_bandwidth<2>(d_result, h_result);
    if (warps == 4) bw = run_bandwidth<4>(d_result, h_result);
    if (warps == 8) bw = run_bandwidth<8>(d_result, h_result);
    if (warps == 16) bw = run_bandwidth<16>(d_result, h_result);
    if (warps == 32) bw = run_bandwidth<32>(d_result, h_result);
