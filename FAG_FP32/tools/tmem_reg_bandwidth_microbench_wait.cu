...
// tcgen05.ld is asynchronous.  Reading a subset of its destination registers
// or placing only a cross-thread fence after a long loop does not define the
// time at which every byte of a tile has reached registers.  This instruction
// explicitly waits for all prior TMEM loads issued by the calling warp.
__device__ __forceinline__ void wait_all_tmem_loads() {
