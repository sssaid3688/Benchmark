...
__global__ static void row_col_rht_gemm_device(
  ...
  static int constexpr NumEpilogueColQuantThreadCount = kEnableRHTColQuant ? (kEnableColOnlyDualQuantPath ? 256 : 128) : 0;
    ...
    if constexpr (kEnableRHTColQuant) {
      ...
      do {
        ...
        for (int k_tile = 0; k_tile < K_TILE_MAX && k_tile + scheduler.tile_n_base() < scheduler.tiles_n(); ++k_tile) {
          bool const process_k_tile = (!kEnableColOnlyDualQuantPath) || ((k_tile & 1) == col_quant_group);
          if (process_k_tile) {
