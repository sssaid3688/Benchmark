...
struct Sm100FmhaFwdMainloopTmaWarpspecializedMxfp8 {
  ...
  // B64 is faster for the on-chip P producer/consumer path than B32/B128 and
  // reduces PV shared-load wavefront conflicts.
  ...
  softmax_step(
    ...
    float block_max_0 = -INFINITY;
    float block_max_1 = -INFINITY;
    #if defined CUTE_ARCH_TCGEN05_TMEM_STAT_ENABLED
    ...
    #endif
    {
      ...
      float row_max_1 = -INFINITY;
      float row_max_2 = -INFINITY;
      float row_max_3 = -INFINITY;
      CUTLASS_PRAGMA_UNROLL
      for (int i = 0; i < 32; i += 2) {
        row_max_0  = ::fmax(row_max_0, tTMEM_LOADrS(i));
        row_max_1 = ::fmax(row_max_1, tTMEM_LOADrS(i+1));
        row_max_2 = ::fmax(row_max_2, tTMEM_LOADrS(i+32));
        row_max_3 = ::fmax(row_max_3, tTMEM_LOADrS(i+33));
      }
      block_max_0 = ::fmax(row_max_0, row_max_1);
      block_max_1 = ::fmax(row_max_2, row_max_3);
      row_max = ::fmax(row_max, row_max_0);
      row_max = ::fmax(row_max, row_max_1);
    ...
    constexpr int kConversionsPerStep = 8;
    ...
    // Derive each dynamic scale from the score-block maximum, then quantize
    // each P value immediately as it is generated by softmax.
    constexpr int kNSFBlk  = 2;
    // Use one dynamic scale for this group's 64 P values and duplicate it
    // across its two 32-element MXFP8 blocks. This keeps row/tile-local
    // dynamic quantization while shortening the scale dependency chain.
    float group_max = ::fmax(block_max_0, block_max_1);
    constexpr float kLog2E4M3Max = 8.807354922057604f;
    int e = __float2int_ru(scale * (group_max - row_max_safe) - kLog2E4M3Max);
    if (e < -127) e = -127;
    if (e > 127) e = 127;
    uint8_t scale_e8m0 = uint8_t(e + 127);
    float scale_inv = __uint_as_float((uint32_t)(127 - e) << 23);

    CUTLASS_PRAGMA_UNROLL
    for (int i = 0; i < size(tTMEM_LOADrS); i += kConversionsPerStep) {
      Array<ElementQK, kConversionsPerStep> in_conv;
      CUTLASS_PRAGMA_UNROLL
      for (int x = 0; x < kConversionsPerStep; x += 2) {
        float2 in = make_float2(tTMEM_LOADrS(i+x), tTMEM_LOADrS(i+x+1));
        float2 out;
        cute::fma(out, scale_fp32x2, in, minus_row_max_scale_fp32x2);
        float p0 = fast_exp2f(out.x);
        float p1 = fast_exp2f(out.y);
        tTMEM_LOADrS(i+x) = p0;
        tTMEM_LOADrS(i+x+1) = p1;
        in_conv[x] = p0 * scale_inv;
        in_conv[x+1] = p1 * scale_inv;
      }
    ...
    //   byte_offset(row, kblock) = (row%32)*16 + (row/32)*4 + kblock; per stage = +sbuf*512.
    {
      ...
      for (int b = 0; b < kNSFBlk; ++b) {
        sf_buf[stage_off + row_off + (kblk_base + b)] = scale_e8m0;
      }
    }
