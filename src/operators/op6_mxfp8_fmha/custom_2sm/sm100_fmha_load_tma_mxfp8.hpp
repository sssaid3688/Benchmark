...
struct Sm100FmhaLoadTmaWarpspecializedMxfp8 {
  ...
  load(
    ...
    if (lane_predicate) {
      ...
      copy(params.tma_load_sfa.with(*tma_barrier, 0), tQgSFA(_, q1_index), tQsSFQ(_, pipeline_q_producer_state.index()));  // [MXFP8]
    }
    ++pipeline_q_producer_state;

    // V1 (+ SFB filler — re-load K1's SF so the KV transaction stays uniform)
    pipeline_kv.producer_acquire(pipeline_kv_producer_state);
    if (lane_predicate) {
      auto tma_barrier = pipeline_kv.producer_get_barrier(pipeline_kv_producer_state);
      copy(params.tma_load_v.with(*tma_barrier, 0), tVgV(_, k_index), tVsV(_, pipeline_kv_producer_state.index()));
      copy(params.tma_load_sfb.with(*tma_barrier, 0), tKgSFB(_, k_index), tKsSFK(_, pipeline_kv_producer_state.index()));  // [MXFP8] filler
    }
    ++pipeline_kv_producer_state;
    k_index += 1;
