...
struct Sm100FmhaFwdMainloopTmaWarpspecializedMxfp8 {
  ...
  mma(
    ...
    Tensor tOrP0 = tOrP;  tOrP0.data() = tOrP0.data().get() + uint32_t(TmemAllocation::P0);
    Tensor tOrP1 = tOrP;  tOrP1.data() = tOrP1.data().get() + uint32_t(TmemAllocation::P1);
    ...
    auto do_pv = [&](int vidx, int buf, bool first_pv) {
      Tensor tOrPk = tOrP;  tOrPk.data() = tOrP.data().get() + warp_uniform(uint32_t(buf ? TmemAllocation::P1 : TmemAllocation::P0));
      if (first_pv) gemm_zero_acc(mma_pv_ts, tOrPk, tOrV(_,_,_,vidx), tOtO0);
      else          gemm_reset_zero_acc(mma_pv_ts, tOrPk, tOrV(_,_,_,vidx), tOtO0);
    };
    // ------------------------------------------------------------------
  ...
  softmax_step(
    ...
    Tensor tScS = tScS_full.compose(make_layout(make_shape(_128{}, _64{})));
