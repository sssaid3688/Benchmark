...
__global__ static void row_col_rht_gemm_device(
  ...
  static constexpr bool kEnableColOnlyDualQuantPath = kEnableRHTColQuant && !kEnableRowQuant;
  ...
  static int constexpr NumEpilogueColQuantGroups = kEnableColOnlyDualQuantPath ? 2 : 1;
  static int constexpr NumEpilogueColQuantThreadCount = kEnableRHTColQuant ? NumEpilogueColQuantGroups * 128 : 0;
  static int constexpr NumEpilogueColQuantThreadCountPerStage = kEnableRHTColQuant ? 128 : 0;
  ...
  bool is_epilogue_col_quant_warp = kEnableColOnlyDualQuantPath ? (warp_idx >= 4 && warp_idx <= 11) : (warp_idx >= 4 && warp_idx <= 7);
  bool is_epilogue_col_quant1_warp = kEnableColOnlyDualQuantPath && (warp_idx >= 8 && warp_idx <= 11);
  bool is_epilogue_row_quant_warp = (!kEnableColOnlyDualQuantPath) && (warp_idx >= 8 && warp_idx <= 15);
  ...
  accumulator_pipeline_params.consumer_arv_count = size(AtomThrShapeMNK{}) * NumEpilogueColQuantThreadCountPerStage;
  ...
  if (is_dma_warp) {
    ...
    if constexpr (kEnableRHTColQuant) {
      ...
      int const col_quant_group = (kEnableColOnlyDualQuantPath && is_epilogue_col_quant1_warp) ? 1 : 0;
      ...
      do {
        scheduler.fetch_next_work(clc_pipeline, clc_pipeline_consumer_state);
        ++clc_pipeline_consumer_state;
        if (col_quant_group != 0 && scheduler.tile_n_base() < scheduler.tiles_n()) {
          ++accumulator_pipe_consumer_state;
        }
        for (int k_tile = col_quant_group;
             k_tile < K_TILE_MAX && k_tile + scheduler.tile_n_base() < scheduler.tiles_n();
             k_tile += NumEpilogueColQuantGroups) {
          ...
          auto qpvscale_ups = cutlass::NumericArrayConverter<ElementAccumulator, TSFD, NumVecs>{}(pvscales_cvted);
          ...
          copy_if(
                  ...
                  tDrSFD_v, tDgSFD_v);
          if (k_tile + 1 < K_TILE_MAX && k_tile + 1 + scheduler.tile_n_base() < scheduler.tiles_n()) {
            ++accumulator_pipe_consumer_state;
          }
        }
        scheduler.update_work_tile_info();
